...
public class B23289 {
    ...
    static int[][] map, plus;
    static boolean[][][][] wall;
    static List<Pos> machines = new ArrayList<>(), checkList = new ArrayList<>();
    ...
    static void work(int i, int j, int dir) {
    ...
    static boolean isWall(int y, int x, int ny, int nx, int machineDir) {
        if (y == ny || x == nx) {
    ...
    static void _adjust(int i, int j) {
        for (int d = 1; d < 5; d++) {
            int ny = i + dy[d];
            int nx = j + dx[d];
            if (!isRange(ny, nx)) continue;
